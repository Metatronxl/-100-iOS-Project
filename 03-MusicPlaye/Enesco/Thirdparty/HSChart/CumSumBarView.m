...
- (void)stockBackGroundLayer
{
    ...
    [_backLayer draw_updateFrame:_barLayer.frame lizard:^(GraphLizard *make) {
        ...
        // 网格横线以及标志文字
        [_timeAry enumerateObjectsUsingBlock:^(NSString *text, NSUInteger idx, BOOL * stop) {
            ...
            make.makeLine.color(__RGB_GRAY).width(0.6);
            make.makeLine.draw();
            NSString *baseText = [NSString stringWithFormat:@"%@ H",[_timeAry[idx] stringValue]];
            make.makeText.text(baseText).font(font).color(txtColor);
//            make.makeText.text(@"test").font(font).color(txtColor);
